...
// };
// //arrow 
const getUsersByEyeColor = (arr, color) => arr.filter(el => el.eyeColor === 'blue' ? el : null)
...
// };
//arrow 
const getUsersByGender = (arr, gender) => arr.filter(el => el.gender === 'male' ?el : null).map(el2 => el2.name)
...
// };
//arrow
const getInactiveUsers = arr => arr.filter(el=> el.isActive === false? el : null)
...
// };
//arrow
const getUserByEmail = (arr, email) => arr.filter(el => el.email === email ? el : null)[0]
...
// };
//arrow
const getUsersWithAge = (arr, min, max) => arr.filter(el => el.age >= min && el.age <= max ? el : null)
...
// };
//arrow
const getUsersByFriend = (arr, name) => arr.filter(el => el.friends.includes(name) ? el : null).map(el2 => el2.name)
...
//         return el.skills
//     }).reduce(function(acc,arr2) {
//         return acc.concat(arr2)
//     }).sort().filter(function (el, index, arr) {
//         if (el !== arr[index + 1]) {
//             return el
//         }
//     })
// };
//arrow
const getAllSkills = arr => arr.map(el => el.skills).reduce((acc,arr2) => acc.concat(arr2).sort().filter((el, index, arr) => el !== arr[index + 1] ? el : null))
